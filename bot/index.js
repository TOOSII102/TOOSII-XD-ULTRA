...
const { normalizeMessageContent, downloadContentFromMessage, downloadMediaMessage, jidNormalizedUser, generateWAMessageFromContent, proto } = require('@whiskeysockets/baileys');
const NodeCache = require('node-cache');
const { isSudoNumber, isSudoJid, getSudoMode, addSudoJid, mapLidToPhone, isSudoByLid, getPhoneFromLid, getSudoList, hasUnmappedSudos } = require('./lib/sudo-store.js');
const db = require('./lib/database.js');
const { setConfigBotId } = require('./lib/database.js');
const { useSQLiteAuthState, getSessionStats } = require('./lib/authState.js');
const { getBotName: _getBotName, clearBotNameCache } = require('./lib/botname.js');
...
const setActiveCommand = (chatId, command, args, sender) => {
    if (chatId) _activeCommandMap.set(chatId, { command: command || '', args: args || [], sender: sender || '', ts: Date.now() });
};
const clearActiveCommand = (chatId) => {
    if (chatId) _activeCommandMap.delete(chatId);
};
const getActiveCommand = (chatId) => {
    if (!chatId) return null;
    const entry = _activeCommandMap.get(chatId);
    if (!entry) return null;
    if (Date.now() - entry.ts > 60000) { _activeCommandMap.delete(chatId); return null; }
    return entry;
};
const buildCommandButtons = () => [];
const { migrateSudoToSupabase, initSudo, setBotId } = require('./lib/sudo-store.js');
const { migrateWarningsToSupabase } = require('./lib/warnings-store.js');
const { detectPlatform } = require('./lib/platformDetect.js');
const { applyFont: _applyFont } = require('./lib/fontTransformer.js');
...
const { handleAutoReact } = require('./toosi-cmds/automation/autoreactstatus.js');
const { handleChannelReact, discoverNewsletters, channelReactManager } = require('./toosi-cmds/channel/channelreact.js');
const { handleReactOwner } = require('./toosi-cmds/automation/reactowner.js');
const { handleReactDev } = require('./toosi-cmds/automation/reactdev.js');
const { handleAutoView } = require('./toosi-cmds/automation/autoviewstatus.js');
const { handleAutoDownloadStatus } = require('./toosi-cmds/automation/autodownloadstatus.js');
const { initializeAutoJoin } = require('./toosi-cmds/group/add.js');
const antidemote = require('./toosi-cmds/group/antidemote.js');
const { isBugMessage: antibugCheck, isEnabled: antibugEnabled, getAction: antibugGetAction } = require('./toosi-cmds/group/antibug.js');
const { checkMessageForLinks: antilinkCheck, isEnabled: antilinkEnabled, getMode: antilinkGetMode, getGroupConfig: antilinkGetConfig, isLinkExempt: antilinkIsExempt, isExcludedType: antilinkIsExcludedType } = require('./toosi-cmds/group/antilink.js');
const { checkMessageForBadWord, isGroupEnabled: isBadWordEnabled, getGroupAction: getBadWordAction } = require('./lib/badwords-store.js');
const { isEnabled: antispamEnabled, getAction: antispamGetAction, checkSpam: antispamCheck } = require('./toosi-cmds/group/antispam.js');
const banCommand = require('./toosi-cmds/group/ban.js');
const { setupWebServer, updateWebStatus } = require('./lib/webServer.js');

// Pre-imported group event modules (avoids dynamic import disk I/O in hot event handlers)
const { handleGroupParticipantUpdate: antidemoteHandler } = require('./toosi-cmds/group/antidemote.js');
const { handleGroupUpdate: antipromoteHandler } = require('./toosi-cmds/group/antipromote.js');
const { isWelcomeEnabled, getWelcomeMessage, sendWelcomeMessage } = require('./toosi-cmds/group/welcome.js');
const { isGoodbyeEnabled, getGoodbyeMessage, sendGoodbyeMessage } = require('./toosi-cmds/group/goodbye.js');
const { handleStatusMention: statusMentionHandler } = require('./toosi-cmds/group/antistatusmention.js');
...
const { initStatusAntidelete, statusAntideleteStoreMessage, statusAntideleteHandleUpdate, updateStatusAntideleteSock } = require('./toosi-cmds/owner/antideletestatus.js');
const { initStatusReplyListener } = require('./lib/statusReplyListener.js');
...
class UltraCleanLogger {
    ...
    static success(...args) {
        ...
        originalConsoleMethods.log(
            `${_GB}[${t}]${_R} ${_G}✔${_R}  ${chalk.greenBright(text)}`
        );
    }
    ...
    try {
        ...
        const sock = makeWASocket({
            ...
            getMessage: async (key) => {
                ...
                try {
                    if (globalThis._ciphertextBlacklist_ref?.has(key.id)) return undefined;
            ...
            if (connection === 'open') {
                ...
                setTimeout(async () => {
                    ...
                    try {
                        ...
                        const ramPct      = Math.min(100, Math.round((_liveRss / _os.totalmem()) * 100));
...
`));
            ...
            try {
                ...
                if (update.action === 'add' && participants.length > 0) {
                    if (await isWelcomeEnabled(groupId)) {
                        ...
                        sendWelcomeMessage(sock, groupId, participants, welcomeMsg, { approvedBy }).catch(() => {});
                    }
                }
                ...
                if (update.action === 'demote' || update.action === 'promote') {
                    ...
                    if (update.action === 'promote') {
                        try { antipromoteHandler(sock, update).catch(() => {}); } catch {}
                    }
                }
            } catch (error) {
        ...
        sock.ev.on('messages.upsert', async ({ messages, type }) => {
            ...
            if (_msgDedupId) {
                ...
                if (_processedMsgIds.has(_msgDedupId) && !_isPendingRetry) return;
                                ...
                                if (isSenderAdmin && alExemptAdmins) {
                                    ...
                                    if (alIsExemptDomain && !alIsExcludedType) {
                                        ...
                                        try {
                                            ...
                                            if (resolved) alSenderNum = resolved;
                                        } catch {}
        ...
        try {
            ...
            try {
                ...
                for (const _f of _files) {
                    if (!_f.startsWith('sender-key-')) continue;
                    const _m = _f.match(/sender-key-(\d+@g\.us)/);
                    if (!_m) continue;
                    _keyStats[_m[1]] = (_keyStats[_m[1]] || 0) + 1;
                }
            } catch {}
            const _lowGroups = Object.entries(_keyStats)
                .filter(([, n]) => n < KEY_SYNC_THRESHOLD)
                .map(([g]) => g);
            if (_lowGroups.length === 0) return;
...
function saveAntiViewOnceConfig(config) {
    try {
        ...
        const localFile = './data/antiviewonce/config.json';
        try {
            if (!fs.existsSync('./data/antiviewonce')) fs.mkdirSync('./data/antiviewonce', { recursive: true });
...
async function handleIncomingMessage(sock, msg) {
    ...
    try {
        ...
        const _presRealContent = msg.message && (
        ...
        );
        {
            ...
            if (_presRealContent && chatId !== 'status@broadcast'
                && (chatId.endsWith('@s.whatsapp.net') || chatId.endsWith('@g.us') || chatId.endsWith('@lid'))
                && _pType) {
                    ...
                    const executeToosiiAICommand = async (cmdName, cmdArgs) => {
                        ...
                        try {
                            ...
                            if (getPresenceType(chatId)) {
                                try { await sock.sendPresenceUpdate('paused', resolvePresenceJid(chatId)); } catch {}
                            }
                        }
                    };
            ...
            if (jidManager.isOwner(msg) && textMsg && textMsg.trim().length > 0) {
                if (senderJid.includes('@lid')) {
                    ...
                    if (_ownerNum && !lidPhoneCache.get(_lidRaw)) {
                        ...
                        if (!OWNER_LID) OWNER_LID = senderJid;
                    }
                }
        ...
        switch (commandName) {
            ...
            case 'help': {
                ...
                const _helpLines = [
                    ...
                    `║  ◇ ${prefixDisplay}av          → anti view-once`,
                    `║  ◇ ${prefixDisplay}members     → member detector`,
                    `║  ◇ ${prefixDisplay}statusstats → status stats`,
                    `║  ◇ ${prefixDisplay}setprefix   → change prefix`,
                    `║  ◇ ${prefixDisplay}ultimatefix → fix owner access`,
                    `║`,
                    `╚═╝`,
                ];
                await sock.sendMessage(chatId, { text: _helpLines.join('\n') }, { quoted: msg });
                break;
            }
            ...
            case 'statusstats': {
                ...
                if (statusDetector) {
                    ...
                    const _ssLines = [
                        ...
                        `║`,
                        `║ ▸ *Status*   : ✅ Active`,
                        `║ ▸ *Detected* : ${_ssStats.totalDetected}`,
                        `║ ▸ *Last*     : ${_ssStats.lastDetection || 'None yet'}`,
                        `║`,
                    ];
                    ...
                    await sock.sendMessage(chatId, { text: _ssLines.join('\n') }, { quoted: msg });
                } else {
                    await sock.sendMessage(chatId, {
            ...
            case 'memberstats': {
                ...
                if (_mbAction === 'on' || _mbAction === 'enable') {
                    if (memberDetector) memberDetector.enabled = true;
                    await sock.sendMessage(chatId, {
                ...
                if (_mbAction === 'off' || _mbAction === 'disable') {
                    if (memberDetector) memberDetector.enabled = false;
                    await sock.sendMessage(chatId, {
                        text: `╔═|〔  MEMBER DETECTOR 〕\n║\n║ ▸ *State* : ❌ Disabled\n║\n╚═|〔 ${getCurrentBotName()} 〕`
                    }, { quoted: msg });
                    break;
                ...
                if (_mbIsGroup) {
                    try {
                        ...
                        const { resolveDisplayWithName: _mbRDWN } = require('./lib/groupUtils');
                        const _mbLines = await Promise.all(_mbShown.map(async (p) => {
                            ...
                            const display   = await _mbRDWN(sock, chatId, p.id || '', p.notify || null).catch(() => {
                                const { phoneNum } = extractParticipantInfo(p, sock);
                                return phoneNum ? `+${phoneNum}` : '[hidden]';
                            });
                        ...
                        const _mbFull = [
                            `╔═|〔  GROUP MEMBERS 〕`,
                            `║`,
                            `║ ▸ *Group*   : ${_mbMeta.subject}`,
                            `║ ▸ *Total*   : ${_mbTotal} members`,
                            `║ ▸ *Admins*  : ${_mbAdminCount}`,
                            `║ ▸ *Showing* : first ${Math.min(_mbLimit, _mbTotal)} of ${_mbTotal}`,
                            `║`,
                            ..._mbLines,
                            `║`,
                            `╚═|〔 ${_mbName} 〕`,
                        ...
                        await sock.sendMessage(chatId, { text: _mbFull }, { quoted: msg });
                    } catch (_mbErr) {
                        await sock.sendMessage(chatId, {
                ...
                if (memberDetector) {
                    ...
                    const _mbLines = [
                        ...
                        `║ ▸ *Cached*  : ${stats.cachedGroups}`,
                        `║`,
                        `║ ▸ *Usage*:`,
                        `║   ${prefixDisplay}members on   → enable`,
                        `║   ${prefixDisplay}members off  → disable`,
                        `║   (use in a group to see participants)`,
                        `║`,
                        `╚═|〔 ${_mbName} 〕`,
                    ];
                    await sock.sendMessage(chatId, { text: _mbLines.join('\n') }, { quoted: msg });
                } else {
                    await sock.sendMessage(chatId, {
                        text: `╔═|〔  MEMBER DETECTOR 〕\n║\n║ ▸ *Status* : ❌ Not initialized\n║\n╚═|〔 ${getCurrentBotName()} 〕`
                    }, { quoted: msg });
            ...
            case 'welcomeconfig': {
                ...
                await sock.sendMessage(chatId, {
                    text: [
                        ...
                        `║`,
                        `║ ▸ *How it works*:`,
                        `║   1. Bot detects new group members`,
                        `║   2. Sends welcome with profile pic`,
                ...
                if (fixResult.success) {
                    await sock.sendMessage(chatId, {
                        text: [
                            ...
                            `║`,
                            `║ ▸ *Result*  : ✅ Applied`,
                            `║ ▸ *Access*  : Full owner access restored`,
                            `║`,
                            `╚═|〔 ${_ufName} 〕`,
                        ].join('\n')
                    }, { quoted: msg });
            ...
            case 'prefixinfo': {
                ...
                await sock.sendMessage(chatId, {
                    text: [
                        `╔═|〔  PREFIX INFO 〕`,
                        `║`,
                        `║ ▸ *Current*   : "${isPrefixless ? 'none' : currentPrefix}"`,
                        `║ ▸ *Default*   : ${DEFAULT_PREFIX}`,
                        `║ ▸ *Prefixless*: ${isPrefixless ? '✅ Enabled' : '❌ Disabled'}`,
                        `║ ▸ *DB Cache*  : ${!!_cache_bot_settings ? '✅' : '❌'} settings | ${!!_cache_prefix_config ? '✅' : '❌'} prefix`,
                        `║`,
                        `║ ▸ *Tip* : Changes persist after restart`,
                        `║`,
                        `╚═|〔 ${_piName} 〕`,
                    ].join('\n')
                }, { quoted: msg });
